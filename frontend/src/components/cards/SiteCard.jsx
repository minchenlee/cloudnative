...
function SiteCard(props) {
  ...
  return (
      <Link to={""} className="relative w-[340px] h-[480px] group flex flex-col items-center rounded-3xl shadow-[2px_4px_8px_1px_rgba(0,0,0,0.25)] overflow-hidden shrink-0 snap-center">
        ...
        <div className={`w-full h-2/5 rounded-3xl overflow-hidden group-hover:blur-sm group-hover:brightness-90 transition duration-500`}>
          <img src={json.img_url} alt="" className="w-full h-full object-cover"/>
        </div>
        <div className="w-full h-3/5 px-6 flex flex-col items-start">
          <p className="text-xl font-medium text-center mt-2">{json.name}</p>
          <div className="flex flex-row gap-4 w-full mt-1 text-base font-semibold text-dark-gray">
            <p className="">{json.inOrOut}</p>
            <p className="">{json.numberOfCourts} 個球場</p>
          </div>
          <div className="mt-11 w-full">
            <StatusGrid weekStatus={json.weekStatus}/>
          </div>
        </div>
      </Link>
  );
